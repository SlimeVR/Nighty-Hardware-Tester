...
use tester::{
    adc, api, logger, options, pio,
    test_executors::{mainboard, TestExecutor},
    Board, TestResult,
};
...
fn main() {
    ...
    spawn(move || {
        ...
        let mut executor = match options.report_type.as_str() {
            "mainboard" => {
                mainboard::MainboardTestExecutor::new(adc, gpio, logger.clone(), options.clone())
            }
            _ => {
                {
                    let mut l = logger.lock().unwrap();
